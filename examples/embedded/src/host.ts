...
const COMMAND = process.argv.slice(2).length
...
class Host {
  ...
  lineBuf = EMPTY;
  pointerInApp = false;
  sockPath = `/tmp/embedded-example-${process.pid}.sock`;
  ...
  async run() {
    ...
    const log = fs.openSync(path.join(HERE, "..", "terminal-browser.stderr.log"), "a");
